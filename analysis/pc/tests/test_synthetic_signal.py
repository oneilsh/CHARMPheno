"""Synthetic known-signal validation for the flat PC topic model — the
"the machine works" gate (plan Task A2).
...
predictive topic. Prediction-Constrained training, guided by the labels, is
free to devote one of its scarce topics to the predictive direction.

We compare three models on a held-out split by ROC AUC:

  1. **PC** (``lam > 0``): the faithful constrained objective.
  2. **Two-stage**: the SAME class with ``lam=0`` (unsupervised LDA-MAP) for the
     representation, then a plain ``LogisticRegression`` on its topic
     proportions — the Hughes baseline PC is meant to beat.
  3. **LR-on-codes**: logistic regression on the raw counts — a sanity
     reference confirming the signal is genuinely present in the data.

The gate asserts PC clears chance by a wide margin AND beats the two-stage
baseline by a real margin. See ``test_model.py`` for the wrapper's unit tests.
...
label slope=28, 180 tokens/doc, 70/30 split, data seed 0. Reference AUCs at
this seed: PC~0.96, two-stage~0.43, LR-on-codes~0.95 (see module test output).
"""
...
def test_pc_recovers_planted_signal_and_beats_two_stage(capsys):
    ...
    # 1) PC (supervised, constrained).
    pc = PCTopicModel(K=K_FIT, C=2, lam=100.0, alpha=1.0, max_iter=600, seed=0)
    pc.fit(Xtr, ytr)
    pc_auc = roc_auc_score(yte, pc.predict_proba(Xte)[:, 1])

    # 2) Two-stage: unsupervised (lam=0) representation -> logistic regression.
    unsup = PCTopicModel(K=K_FIT, C=2, lam=0.0, alpha=1.0, max_iter=600, seed=0)
    unsup.fit(Xtr, ytr)  # label ignored at lam=0
    ...
    with capsys.disabled():
        print(
            f"\n[synthetic signal] heldout ROC AUC — "
    ...
    assert pc_auc > 0.72, f"PC heldout AUC {pc_auc:.3f} not above chance gate"
    # PC beats the two-stage baseline by a real margin (strict Hughes result).
    assert pc_auc > two_stage_auc + 0.02, (
