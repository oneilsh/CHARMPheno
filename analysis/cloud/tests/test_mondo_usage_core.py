...
PORTED VERBATIM from main's `analysis/cloud/tests/test_mondo_usage.py` alongside
`mondo_usage_core` (see that module's provenance docstring). The ONLY edit is the
import line — `mondo_usage_cloud` -> `mondo_usage_core` — so a later unification
of the two histories can diff these files and find them identical.
"""
import mondo_usage_core as m
