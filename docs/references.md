...
- **Jiang, M.J. Zhang, Y. Zhang, Durvasula, Inouye, Holmes, Price & McVean (2023).** Age-dependent Topic Modelling (ATM) of Comorbidities in UK Biobank Identifies Disease Subtypes with Differential Genetic Risk. *Nature Genetics* 55:1854–1865.
  https://doi.org/10.1038/s41588-023-01522-8
  — a low-rank **topic model of longitudinal EHR disease codes as a function of age**: each patient gets weights over comorbidity "topics," and disease subtypes defined by topic membership show differential polygenic risk. The **closest topic-model-of-EHR neighbor to CharmPheno** from the ALADYNOULLI group — but PheCode-level, age-indexed, genetics-subtype-oriented, and NOT ontology-gated or diagnosis-oriented. Code: github.com/Xilin-Jiang/ATM. **(landscape)**
- **Urbut, Ding, Nakao, Koyama, Misra, Jiang, … Gusev, Natarajan & Parmigiani (2026).** A Bayesian Framework for Longitudinal EHR and Genetic Discovery (ALADYNOULLI). *Nature* (2026). (preprint medRxiv 2024.09.29.24314557)
  https://doi.org/10.1038/s41586-026-10780-5
  — dynamic Bayesian latent-signature model over longitudinal **PheCode** diagnoses + age + polygenic risk: **GP-time-varying** per-patient signature loadings, a survival/Bernoulli likelihood ("mixture of probabilities, not probability of a mixture" — the Poisson/Bernoulli-factorization contrast to allocation-based LDA; a *feature for forecasting comorbid load, but the simplex it drops is exactly what conditional diagnosis wants*), and joint genetics. 21 (preprint: 20) signatures over 348 diseases across UK Biobank + MGB + All of Us (n>683k). Fit by **gradient MAP (Adam/PyTorch)**, scaled by subset-averaging under a shared spectral-clustering init (not distributed VI or a true posterior); the PheCode vocabulary sidesteps the OMOP/SNOMED harmonization + licensing burden CharmPheno carries. The nearest interpretable-Bayesian-latent-EHR neighbor **at Nature scale — but a different tribe (statistical genetics), altitude (coarse population discovery + GWAS), and objective (forecasting) than CharmPheno's ontology-gated rare-disease diagnosis**. Lineage: mash (Urbut, Wang, Carbonetto & Stephens, *Nat Genet* 51:187–195, 2019 — multivariate-adaptive-shrinkage effect-sharing across conditions) → ATM (above) → dynamic CAD risk (MSGene, *Nat Commun* 15, 2024, 10.1038/s41467-024-49296-9; "Dynamic Importance of Genomic and Clinical Risk for CAD Over the Life Course," *Circ Genom Precis Med* 2025) → ALADYNOULLI. **(landscape)**
