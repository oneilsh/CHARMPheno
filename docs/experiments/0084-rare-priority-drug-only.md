...
Exp 0081 variation (config-only). Same as 0081 but drop measurement (condition + drug only). Isolates drug's solo contribution — does the treatment-defined specialization (temporal arteritis, MG) survive without measurement?

```bash
cd ~/repos/CHARMPheno && git pull origin claude/spectral-anchor-topic-k-200nqp && \
  CHARM_SPARK_CONF='spark.locality.wait=0s' make -C analysis/cloud exp ID=84
```
