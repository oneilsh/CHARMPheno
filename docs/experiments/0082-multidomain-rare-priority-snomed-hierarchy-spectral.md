...
# exp 0082 — Hierarchy+roll-up with SPECTRAL init (clears the under-fit confound)

Identical to exp 0080 (SNOMED hierarchy + roll-up) except **spectral init**. exp
0080 collapsed case-finding ~3× but left 81/424 starved topics under random init —
i.e. the rare anchors were under-fit, confounding "pooling hurts" with "the fit
was degenerate" (insight 0080, revised). Spectral init gave 0 starved topics in
the flat 0079 fit; if it does so here and the AP recovers toward flat (~0.020),
