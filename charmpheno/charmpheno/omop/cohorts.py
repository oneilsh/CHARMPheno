...
# Expanded rare-disease anchor set for the pooling experiment (ADR 0039): the six
# rare6 anchors (pinned) + prioritised rare diseases from the Monarch dismech
# #1079 list, mapped MONDO->OMOP and kept by the objective floor(50)/ceiling(10000)
# /nesting criteria. Generated by analysis/cloud/anchor_neighborhoods.py from
# analysis/cloud/anchor_selection_data/candidates_with_counts.tsv; labels in
# frozen_anchors.tsv. Concept-ids, not names, are authoritative. Order: rare6
# first, then prioritised by descending coded-patient count.
...
COHORT_METADATA: dict[str, dict[str, str]] = {
    ...
    "population_rare_priority": {
        ...
        "label": "Population + Expanded rare-disease forest (gated)",
        "description": (
            "The whole population as a shared clean background, with an expanded "
            "rare-disease foreground: the six rare6 anchors plus prioritised rare "
            "diseases from the Monarch dismech #1079 list (mapped MONDO->OMOP), "
            "kept by objective floor/ceiling/nesting criteria (ADR 0039). The set "
            "spans several ontology clusters (vasculitis + rare6 autoimmune, "
            "neuroimmune, neurodegenerative, cardiac-genetic) plus isolates, so a "
            "shared placement model can be tested for pooling across related "
            "diseases. Disjoint, one document per person: a person with a first dx "
            "under ANY anchor gets the post-diagnosis window and "
            "source_cohort='rare_priority'; everyone else gets a deterministic "
            "random event-anchored window and source_cohort='general'."
        ),
    },
    "population_sparse": {
