...
def test_build_correlation_json_emits_eta_scale_as_float():
    ...
    (corpus_eta_scale_gated_rdd/corpus_eta_scale_gated). When given, it is
    emitted as "eta_scale": float(c)."""
    part = TopicBlockPartition(group_var="g", background_k=2,
                               foreground=(("A", 1), ("B", 1)))
    R = np.array([[1.0, 0.3, 0.2, np.nan],
                  [0.3, 1.0, 0.1, np.nan],
...
def test_eta_scale_diagnostic_omitted_when_none():
    ...
    'eta_scale_diagnostic' key entirely (mirrors eta_scale omission)."""
    part = TopicBlockPartition(group_var="g", background_k=2,
                               foreground=(("A", 1), ("B", 1)))
    R = np.array([[1.0, 0.3, 0.2, np.nan],
                  [0.3, 1.0, 0.1, np.nan],
