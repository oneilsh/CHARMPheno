"""Tests for the value-aware measurement source table in load_omop_bigquery.

Pure tests (select-cols shape, supported-table membership, cohort rejection) run
without Spark. The cascade behavior test drives ``_load_measurement`` through a
stub ``_read`` (the reason it takes ``_read`` as a parameter) so it exercises the
real Spark tokenization against in-memory `measurement` / `concept` frames without
a BigQuery read.
"""
...
def test_measurement_is_a_supported_source_table_and_concept_type():
    from charmpheno.omop import bigquery as bq
    assert "measurement" in bq._SUPPORTED_SOURCE_TABLES
...
def test_measurement_rejects_cohort_filtering():
    ...
    with pytest.raises(ValueError, match="cohort filtering requires a condition"):
        load_omop_bigquery(spark=None, cdr_dataset="p.d", billing_project="b",
                           source_table="measurement", cohort="population_glp1")
