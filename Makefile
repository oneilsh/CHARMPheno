...
help:
	...
	@echo "  test             - Run unit tests in both packages (the dev loop, ~3.5 min)"
	@echo "  test-all         - Run unit + @slow integration tests (~18 min). Run at"
	@echo "                     branch-merge / walkthrough time, NOT every iteration:"
	@echo "                     @slow is ~82% of runtime and rarely breaks on routine"
	@echo "                     edits (see the marker note in spark-vi/pyproject.toml)"
